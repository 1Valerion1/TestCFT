...
//        temp = text.split(" ");
       ...
       for (int j = 0; j < args.length; j++) {
        ...
        if (temp.length < 3) {
         ...
         //read.close();
        int i = 0;
        //ѕровер€ем введенный тип сортировки
        if (temp[i].equals("-d")) {
            TypeSort = "-d";
            i++;
    ...
    // ѕроверка соответстви€ выбранного типа данных типу данных в файле
    public static String checkArrayType(String[] arr, String expectedType) {
        if (expectedType.equals("-i")) {
            ...
            try {
                int intArr[] = Arrays.stream(arr).mapToInt(value -> Integer.parseInt(value)).toArray();
                System.out.println("¬ы пытаетесь отсортировать массив целых чисел как массив строк." + " “ип сортируемых данных будет изменен.");
                return "-i";
            } catch (NumberFormatException e) {
                return expectedType;
            }
        } else {
            System.out.println("¬ы указали неверный тип входных данных." + " ƒанные будут отсортированы по умолчанию как массив строк.");
            return "-s";
        }
        return expectedType;
    }
    ...
    public static void mergeSort(int[] array, int[] support, int startIndex, int endIndex, String TypeSort) {
        if (startIndex >= endIndex) { // услови€ окончани€ нашей рекурсии
            return;
        }
        ...
        merge(array, support, startIndex, h, h + 1, endIndex, TypeSort); // сли€ние подпоследовательностей
    }
    ...
    public static <T> void mergeSort(T[] array, T[] support, Comparator<T> comp, int startIndex, int endIndex, String TypeSort) {
        ...
        merge(array, support, comp, startIndex, h, h + 1, endIndex, TypeSort);// сли€ние подпоследовательностей
    }
